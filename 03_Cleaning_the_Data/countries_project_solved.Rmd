...
```{r}
un_df <- un_df %>% 
  separate(life_expectancy_at_birth_females_males_years, 
           sep = "/", 
...
```{r}
un_df <- un_df %>% 
  mutate(life_expectancy_at_birth_females = as.numeric(life_expectancy_at_birth_females),
...
```{r}
countries_df <- inner_join(un_df, literacy_df, 
                           by = c("country" = "entity"))
```
